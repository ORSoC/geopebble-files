...
class MASKSET(object):
    ...
    def xsvf(self, sawTDO=False, first=True, last=True):
        """Output XSVF for this mask (only for SDR currently)"""
        setxsdrsize(self.size)

        # Divide into begin/continue/end and with or without TDO
        # Note: switch between XSDRC and XSDRTDOC to emulate mask
        # (done with splitbymask()). The B/C/E commands ignore XTDOMASK.
        if sawTDO and any(self.mask):
            # Will check TDO, set up mask
            if first:
                command=XSDRTDO if last else XSDRTDOB
            else:
                command=XSDRTDOE if last else XSDRTDOC
            if command==XSDRTDO:
                setxtdomask(self.mask)
            elif not all(self.mask):
                raise ValueError("Split XSDR commands do not support TDO masks.")
            tdo=self.tdo
        else:
            # No TDO checking
            if first:
                command=XSDR if last else XSDRB
            else:
                command=XSDRE if last else XSDRC
            tdo=None

        cmdbuf[0] = command
        output.write( cmdbuf )
        obuf = makeXSVFbytes( self.tdi )
        output.write( obuf )
        if tdo:
    ...
    def __getitem__(self, key):
        # Allow slicing a subset in order to split SDR commands
        subset=MASKSET(self.name)
        subset.mask=self.mask[key]
        subset.smask=self.smask[key]
        subset.tdi=self.tdi[key]
        subset.tdo=self.tdo[key]
        subset.size=len(subset.tdi)
        return subset
    def splitbymask(self, maxsize=None):
        parts=[]
        begin=0
        i=1
        while i<self.size:
            # Measure one group of mask 1/0
            while i<self.size and self.mask[i]==self.mask[begin]:
                i+=1
                if i-begin==maxsize:
                    break   # don't produce larger chunks
            parts.append(self[begin:i])
            begin=i
        # Return parts in shift order, i.e. LSB first
        parts.reverse()
        return parts
...
def setxsdrsize(newsize):
        ...
        if xsdrsize != newsize:
                ...
                output.write( obuf )
def setxtdomask(mask):
        global xtdomask
        if xtdomask != mask:
                xtdomask = mask[:]   # Note: should copy!
                cmdbuf[0] = XTDOMASK
                output.write( cmdbuf )
    ...
    while 1:
        ...
        if tokVal in shiftOps:
            ...
            if shiftOp == 'SIR':
                ...
                if hdr.size:
                    parts.append(hdr)
                # Split middle section if too large
                # TODO: merge into a masked shift if hdr+sdr+tdr is small?
                splitsize=8*64
                parts.extend(sdr.splitbymask(splitsize))
                for (i,part) in enumerate(parts):
                    part.xsvf(sawTDO, i==0, i==len(parts)-1)
