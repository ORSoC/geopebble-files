...
class BSL5(object):
    ...
    def memory_write(self, address, data):
        ...
        if self.options.norambsl and self.inflash(address):
            ...
            #assert len(data)%4==0 and address%32==0, "Unaligned write (need 4 byte)"
            # Cannot rely on RAM-based BSL and need flash write routine
            split_size=32
            while data:
                block, data = data[:split_size], data[split_size:]
                self.BSL_RX_DATA_BLOCK_FAST(self.writeflash_args, struct.pack('<IH', address, len(block)) + block)
